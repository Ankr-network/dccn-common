...
func SetValidator(ip, port, pubkey, power, admin_priv_key string) (err_ret error) {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "val_nonce")))
	if err != nil {
		return err
	}
	qres := res.Response
...
func GetBalance(ip, port, address string) (balance string, err_ret error) {
	...
	// curl  'localhost:26657/abci_query?data="bal:1234567890123456789012345678901234567890"'
    	...
    	res, err := cl.ABCIQuery("/store/account", cmn.HexBytes(fmt.Sprintf("%s:%s", "bal", address)))
	if err != nil {
		return "", err
	}
	qres := res.Response
...
func GetAllDatacenterIds(ip, port string) (allIDs string, err_ret error) {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "all_crts")))
	if err != nil {
		return "", err
	}
	qres := res.Response
...
func SendCoins(ip, port, priv_key, from_address, to_address, amount string) (hash string, err error) {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/account", cmn.HexBytes(fmt.Sprintf("%s:%s", "bal", from_address)))
	if err != nil {
		return hash, err
	}
	qres := res.Response
...
func GetStake(ip, port string) (stake string, err_ret error) {
	...
	// curl  'localhost:26657/abci_query?data="stk"'
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "stk")))
	if err != nil {
		return "", err
	}
	qres := res.Response
...
func SetMeteringCert(ip, port, op_priv_key, dc_name, cert_pem string) error {
	...
	pemB64 := base64.StdEncoding.EncodeToString([]byte(cert_pem))
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "set_crt_nonce")))
	if err != nil {
		return err
	}
	qres := res.Response
...
func RemoveMeteringCert(ip, port, op_priv_key, dc_name string) error {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "rmv_crt_nonce")))
	if err != nil {
		return err
	}
	qres := res.Response
...
func GetMeteringCert(ip, port, dc_name string) (pem string, err error) {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s:%s", "crt", dc_name)))
	if err != nil {
		return "", err
	}
	qres := res.Response
...
func SetOpKey(ip, port, keyname, value, admin_priv_key string) error {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s", "admin_nonce")))
	if err != nil {
		return err
	}
	qres := res.Response
...
func SetMetering(ip, port, priv_key_pem, dc, ns, value string) error {
	...
	if err != nil {
    	...
    	res, err := cl.ABCIQuery("/store/main", cmn.HexBytes(fmt.Sprintf("%s:%s:%s", "mtr", dc, ns)))
	if err != nil {
		return err
	}
	qres := res.Response
